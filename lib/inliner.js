 ...
 * file to process.
 *
 * Returns a promise. On resolution, it resolves with a string containing HTML with inlined JS and CSS.
 */
...
module.exports = function(conf) {
  return new Promise(function(resolve, reject) {
    ...
    var dirname = path.dirname(conf.fname) + path.sep,
        sre = /<script.+?src=["'](?!https{0,1}:\/\/)([^"']+?)["'].*?>/ig,
        srcre = / *src=["'][^"']+?["']/i,
        cre = /<link.+?href=["'](?!https{0,1}:\/\/)([^"']+?)["'].*>/ig,
        hrefre = / *href=["'][^"']+?["']/i,
        crerel = / *rel=["'][^"']+?["']/i,
        cretag = /<link/i,
        filecontent,
     ...
     */
    function getIncludedContent(fileName) {
     ...
     * Find all the script and link tags, filter any that are
     * not local, then read them in. Once all have been read,
     * call inlineLocalContent to do the inlining.
     */
    fs.readFile(conf.fname, {encoding: 'utf8'}, function(err, data) {
      ...
      if (data) {
        ...
        if (files.length === 0) {
          ...
          Promise.settle(files.map(function(itm) {
            return getIncludedContent(itm);
          })).then(inlineLocalContent);
        }
      } else {
        reject(err);
      }
